...
T sanitise_input(
    ...
    std::function<bool(T&)>&& validator = [](T&)-> bool {return true;}
)
{
    T in_val;
...
T sanitise_input_with_context(
    std::string initial_prompt,
    std::string secondary_prompt,
    std::function<bool(T&, void*&)>&& validator = [](T&, void*)-> bool {return true;},
...
T1 sanitise_input_with_cast(
    std::string initial_prompt,
    std::string secondary_prompt,
    std::function<bool(T2&)>&& validator = [](T2&)-> bool {return true;}
)
{
    T2 in_val;
    std::cout << initial_prompt;
